	...
	public class DataManager
	{
		...
		public void setData(AllData allData)
		{
			...
			for (int p = 0; p < allData.General.TrainingPr; p++)
			{
				...
				{
					data_p[p].Intern[i] = new InternInfo(data_p[p].General.Hospitals, data_p[p].General.Disciplines, data_p[p].General.TimePriods, data_p[p].General.DisciplineGr, allData.General.TrainingPr, data_p[p].General.Region);
					data_p[p].Intern[i].ProgramID = allData.Intern[i].ProgramID;
					if (data_p[p].Intern[i].ProgramID == p)
					{
						data_p[p].Intern[i].isProspective = allData.Intern[i].isProspective;
						...
						for (int d = 0; d < data_p[p].General.Disciplines; d++)
						{
							for (int h = 0; h < data_p[p].General.Hospitals; h++)
							{
								data_p[p].Intern[i].Abi_dh[d][h] = allData.Intern[i].Abi_dh[d][h];
								data_p[p].Intern[i].Fulfilled_dhp[d][h][p] = allData.Intern[i].Fulfilled_dhp[d][h][p];
								data_p[p].Intern[i].FHRequirment_d[d] = allData.Intern[i].FHRequirment_d[d];
								data_p[p].Intern[i].overseaReq = allData.Intern[i].overseaReq;
								for (int t = 0; t < data_p[p].General.TimePriods; t++)
								{
									data_p[p].Intern[i].OverSea_dt[d][t] = allData.Intern[i].OverSea_dt[d][t];
									data_p[p].Intern[i].Ave_t[t] = allData.Intern[i].Ave_t[t];
								}
								for (int g = 0; g < data_p[p].General.DisciplineGr; g++)
								{
									data_p[p].Intern[i].DisciplineList_dg[d][g] = allData.Intern[i].DisciplineList_dg[d][g];
									data_p[p].Intern[i].K_AllDiscipline = allData.Intern[i].K_AllDiscipline;
									data_p[p].Intern[i].ShouldattendInGr_g[g] = allData.Intern[i].ShouldattendInGr_g[g];

								}
							}
							data_p[p].Intern[i].Prf_d[d] = allData.Intern[i].Prf_d[d];
						}
						// region 
						for (int r = 0; r < data_p[p].General.Region; r++)
						{
							data_p[p].Intern[i].TransferredTo_r[r] = allData.Intern[i].TransferredTo_r[r];
						}

						for (int h = 0; h < data_p[p].General.Hospitals; h++)
						{
							data_p[p].Intern[i].Prf_h[h] = allData.Intern[i].Prf_h[h];
						}
						data_p[p].Intern[i].wieght_ch = allData.Intern[i].wieght_ch;
						data_p[p].Intern[i].wieght_d = allData.Intern[i].wieght_d;
						data_p[p].Intern[i].wieght_h = allData.Intern[i].wieght_h;
						data_p[p].Intern[i].wieght_w = allData.Intern[i].wieght_w;
					}
					else
					{
						data_p[p].Intern[i].isProspective = allData.Intern[i].isProspective;
						...
						for (int d = 0; d < data_p[p].General.Disciplines; d++)
						{
							for (int h = 0; h < data_p[p].General.Hospitals; h++)
							{
								...
								{
									data_p[p].Intern[i].DisciplineList_dg[d][g] = allData.Intern[i].DisciplineList_dg[d][g];
									data_p[p].Intern[i].K_AllDiscipline = 0;
									data_p[p].Intern[i].ShouldattendInGr_g[g] = 0;
