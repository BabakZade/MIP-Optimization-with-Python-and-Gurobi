		...
		static void Main(string[] args)
		{
			...
			for (int mm = 0; mm < minpercent.Length; mm++)
			{
				for (int g = 0; g < 9; g++)
				{
					for (int i = 0; i < InstanceSize; i++)
					{
						ReadInformation read = new ReadInformation(allpathTotal.CurrentDir, "Complexity", "NHAScBL\\Scenario_" + minpercent[mm].ToString("0.00"), "G_" + (g + 1).ToString(), "Instance_" + i + ".txt");
						read.data.AlgSettings.bucketBasedImpPercentage = minpercent[mm];
