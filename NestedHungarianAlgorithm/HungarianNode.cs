...
namespace NestedHungarianAlgorithm
{
    ...
    public class HungarianNode
    {
        ...
        public double[] preprocessCapacity_d;
        public int[][][] Disc_iwh;
        public int[][] AvAcc_rt;
        ...
        public void Initial()
        {
            ...
            TotalDiscInGr_ig = new int[Interns][];
            for (int i = 0; i < Interns; i++)
            {
                ...
                for (int g = 0; g < DisciplineGr; g++)
                {
                    ...
                    else
                    {
                        ...
                        TotalDiscInGr_ig[i][g] = parentNode.TotalDiscInGr_ig[i][g];
                    }
                }
            }
            ...
            for (int d = 0; d < Wards; d++)
            {
                ...
                for (int h = 0; h < Hospitals; h++)
                {
                    if (isRoot)
                    {
                        ...
                        DemMaxYearly_wh[d][h] = data.Hospital[h].HospitalMaxYearly_w[d];
                    }
                    else
                    {
            ...
            for (int w = 0; w < Wards; w++)
            {
                for (int h = 0; h < Hospitals; h++)
                {
                    ...
                    if (DemMaxYearly_wh[w][h] < DemMax_wth[w][TimeID][h])
                    {
                        TotalAvailablePosition += DemMaxYearly_wh[w][h];
                        for (int dd = 0; dd < DemMaxYearly_wh[w][h]; dd++)
                        {
                ...
                for (int j = 0; j < TotalAvailablePosition + Interns + Interns; j++)
                {
                    ...
                    if (j < TotalAvailablePosition)
                    {
                        ...
                        else
                        {
                            ...
                            for (int g = 0; g < DisciplineGr; g++) // considering course credite in the selection
                            {
                                if (data.Intern[i].DisciplineList_dg[discIn][g])
                                {
                                    double value = (double)TotalDiscInGr_ig[i][g] / discGrCounter_ig[i][g]; // the ration of total availble discipline in a group to total available credit that should be cleared
                                    if (value <= 1) // when the CC will have impact
                                    {
                                        CostMatrix_i_whDem[i][j] *= data.Discipline[discIn].CourseCredit_p[data.Intern[i].ProgramID];
                                    }
                                    else // when the CC is not important
                                    {
                                        CostMatrix_i_whDem[i][j] /= data.Discipline[discIn].CourseCredit_p[data.Intern[i].ProgramID];
                                    }
                                    break;
        ...
        public void setTakingPercentageMotivation()
        {
            ...
            for (int i = 0; i < Interns; i++)
            {
                    for (int j = 0; j < TotalAvailablePosition ; j++)
                    {
                        ...
                        {
                            int discIn = Disc_iwh[i][((PositionMap)MappingTable[j]).WIndex][((PositionMap)MappingTable[j]).HIndex];
                            // we do not enough capacity 
                            // we first assign to the interns that they really want the position
                            if (discIn >= 0 && preprocessCapacity_d[discIn] < 1  && data.Intern[i].takingDiscPercentage[discIn] < 0.95)
                            {
                                double motivation = 1 + 1/data.Intern[i].takingDiscPercentage[discIn];
                                if (data.Hospital[((PositionMap)MappingTable[j]).HIndex].Hospital_dw[discIn][((PositionMap)MappingTable[j]).WIndex])
                                {
                                    CostMatrix_i_whDem[i][j] /= motivation;
                                }
        ...
        public int get_DisciplineFromWard(int theI, int theW, int theH)
        {
            ...
            for (int d = 0; d < Disciplins; d++)
            {
                ...
                // if there is no capcity
                bool noCap = false;
                for (int t = TimeID; t < TimeID + data.Discipline[d].Duration_p[data.Intern[theI].ProgramID]; t++)
                {
                    if (DemMax_wth[theW][t][theH] <= 0)
                    {
                        // this discipline deos not have capacity
                        noCap = true;
                    }
                }
                if (noCap)
        ...
        public void capacityBasedPreprocess()
        {
            new ArrayInitializer().CreateArray(ref preprocessCapacity_d, data.General.Disciplines, 0);
            for (int i = 0; i < Interns; i++)
            {
                for (int d = 0; d < Disciplins; d++)
                {
